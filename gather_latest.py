...
## diy option
fill_zero_bit = int(os.getenv("fill_zero_bit", 8))
...
for f in filter_files:
    ...
    target_path_i=target_path.replace("YYYY_MM",datetime.datetime.strftime(date,"%Y_%m"))
    ...
    # write to 1 csv
    if not os.path.exists(target_path_i):
        df.to_csv(target_path_i, index=False)
    else:
        df.to_csv(target_path_i, mode='a', header=False,  index=False)
